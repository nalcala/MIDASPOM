...
int main(int argc, char ** argv)//takes the path to an input file as argument
{
  ...
  for(ie=stloop; ie<endloop; ie++){
	...
	for(k=0;k<nextid;k++){
	  SPec += P[ j*nextid + k];
	  //printf("%e\t",P[j*nextid + k]);//SPe OK, SPc weird
	  //if(P[j*nextid + k]==0) printf("error!");
	}
	//printf("\n");
      }
	...
	for(k=0;k<npstates[j-1];k++){//for each possible state at time j-1
	  for(l=0;l<npstates[j];l++){//for each possible state at time j
	    ...
	    //printf("%d: Pcur[%d]=%e\t",j, k*npstates[j] + l,Pcur[k*npstates[j] + l] ); //not always 1
	  }//l
	}//k
	...
	/*double SPcur=0;
	int nSPcur=0;
	for(l=0;l<npstates[0];l++){
	  for(k=0;k<npstates[j];k++){
	    SPcur += Pold[ l*npstates[j] + k];
	    if(Pold[ l*npstates[j] + k]==0) nSPcur ++;
	  }
	  }*/
	...
	//printf("bef-endloop-Pold[%d]=%lf\t", 0,log(Pold[0]) ); //always 1?
	  ...
	  for(l=0;l<npstates[tmax-1];l++){
	    //printf("Lik-Pold[%d]=%lf\t", k*npstates[tmax-1]+l,log(Pold[k*npstates[tmax-1] + l]) ); //always 1?
	    Lik[ie][ic] += Pold[k*npstates[tmax-1] + l]*priorst[k];
	    //if(Pold[k*npstates[tmax-1] + l]>0) printf("Lik[%d][%d]temp=%.30e\n",ie,ic,Pold[k*npstates[tmax-1] + l]); //=> always -Inf?
	  }//l
      }//k
      free(Pold);
