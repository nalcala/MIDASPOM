...
int main(int argc, char ** argv)//takes the path to an input file as argument
{
  ...
  char* fout2 = "posterior_patches.txt";
  char* Jname  = "J.txt";
  double d = 100;
  ...
  opterr = 0;
  while ((cget = getopt (argc, argv, "m:p:d:i:J:o:q:e:c:f:")) != -1)
    switch (cget)
      {
      case 'm':
        a = 1.0/atof(optarg);
  ...
  unsigned int* spiall = calloc(nstates,sizeof(unsigned int));
  ...
  double Likoc[tmax][n+1];
  double Lik[tmax][n];
  for(i=0;i<tmax;i++){
    for(j=0;j<n+1;j++) Likoc[i][j] = 0;
    for(j=0;j<n;j++) Lik[i][j] = 0;
  }
  setbuf(stdout, NULL);
  ...
  for(j=0;j<nstates;j++){
    ...
    compPePc(Pe, Pc, D, piall, e, pCtmp, M, n, nstates, j);
  }//j
  /*double SPe=0,SPc=0;
    for(j=0;j<nstates;j++){
    for(k=0;k<nstates;k++){
  ...
  */
  ...
  for(k=0;k<nstates;k++){
    Likoc[0][spiall[k]] += exp( log(Pold[k]) + log(C)); //+= Pold[k*nstates + l]*priorst[k];
    for(j=0;j<n;j++) Lik[0][j] += exp( log(Pold[k]) + log(piall[k][j]) + log(C)); 
  }//k
  for(k=0;k<n+1;k++) Likoc[0][k] = log(Likoc[0][k]);
  for(k=0;k<n;k++) Lik[0][k] = log(Lik[0][k]);
  ...
  for(j=1; j<tmax; j++){//for each year
    ...
    cblas_dgemm(CblasRowMajor,CblasNoTrans,CblasNoTrans, 1, nstates, nstates, C, Pold, nstates, Ptmp2, nstates, 0, Ptmp, nstates);
    /*SP=0;
      for(l=0;l<nstates;l++){
      for(k=0;k<nstates;k++){
      SP += Ptmp[ l*nstates + k];
    ...
    for(k=0;k<nstates;k++){
      //for(l=0;l<nstates;l++){
      //printf("%d->%d, s=%d\n",k,l,spiall[l]);
      Likoc[j][spiall[k]] += exp( log(Pold[k]) + log(C)); //+= Pold[k*nstates + l]*priorst[k];
      for(l=0;l<n;l++) Lik[j][l] += exp( log(Pold[k]) + log(piall[k][l]) + log(C)); 
      //Lik[ie][ic]      += exp( log(Pold[k*nstates + l]) + log(C));
      //if(Pold[k*nstates + l]>0) printf("Lik[%d][%d]temp=%.30e\n",ie,ic,Pold[k*nstates + l]); //=> always -Inf?
      //}//l
    }//k
    for(k=0;k<n+1;k++) Likoc[j][k] = log(Likoc[j][k]);
    for(k=0;k<n;k++) Lik[j][k] = log(Lik[j][k]);
  }//j
  ...
  if(my_id!=root_process){
	...
	Ltot += exp(Likoc[k][l]);
      ...
      for(l=0; l<n+1; l++){
	Likoc[k][l]= exp(Likoc[k][l])/Ltot;
      }
      for(l=0; l<n; l++){
	Lik[k][l]= exp(Lik[k][l])/Ltot;
      }
    }
    //print results
    FILE * fe;
    ...
    printf("Writing output in file %s... ",fout2);
    fe=fopen(fout2,"wb");  
    for(i=0; i<tmax; i++){
      for(j=0; j<n; j++){
	//if(Ltot==0) fprintf(fe,"%.20lf\t", Lik[i][j] );
