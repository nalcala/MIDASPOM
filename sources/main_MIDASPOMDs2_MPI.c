...
int main(int argc, char ** argv)//takes the path to an input file as argument
{
  ...
  unsigned int nstates = 0; //pow(2,nvar); //number of possible states
  unsigned int maxnstates = 0; //pow(2,nvar); //number of possible states
  double thres = 0;
  
  int c;
  
  opterr = 0;
  while ((c = getopt (argc, argv, "m:p:d:i:J:o:s:l:h:u:v:f:t:n:")) != -1)
    switch (c)
      {
      case 'm':
        a = 1.0/atof(optarg);
  ...
  unsigned int nstatestot = pow(2,nvar);
  ...
  //all possible states
  unsigned int** piall = malloc(nstatestot*sizeof(unsigned int*));
  unsigned int piind[nstatestot];
  double prpiall[nstatestot];
  double prtmp ;
  unsigned int pialltmp[n];
  
  //for(i=0; i<nstatestot; i++) piind[i] = i;//piall[nstates][n]; 
  //shuffle(piind ,nstatestot); //get nstates random states
  nstates=0;
  for(i=0; i<nstatestot; i++) piall[i] = malloc(n*sizeof(unsigned int));//piall[nstates][n]; 
  for(i=0; i<nstatestot; i++){
    //int ii = piind[i];
    prtmp = ylik(Y,J,0,n,p,piall[i]);
    unsigned int jtmp = 0;
    for(j=0; j<n;j++){
    ...
    for(j=0; j<tmax; j++) prtmp += ylik(Y,J,j,n,p,pialltmp);
      
    if(prtmp>thres){
      printf("pr(%d)=%lf\t",i,prtmp);
      for(j=0; j<n;j++) piall[nstates][j] = pialltmp[j];
      nstates++;
      if(nstates==maxnstates) break;
    }
  }
