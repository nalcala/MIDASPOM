...
int main(int argc, char ** argv)//takes the path to an input file as argument
{
  ...
  for(i=0; i<tmax; i++){
	...
	if(piobs[i][j]>-1){
	  pstates[i][k] += piobs[i][j]*pow(2,nvar-jtmp-1); // to change if some sites always at 0
	}else{
	  st1 = npstates[i]/pow(2,s1);
	  pstates[i][k] += (k/st1%2)*pow(2,nvar-jtmp-1);
	  if(i==0){
  ...
  printf("Starting parallel likelihood computation\n");
  unsigned int ie,ic;
  ...
  for(i=0; i<nstep; i++){
    for(j=0; j<nstep; j++){
      fprintf(fe,"%.20lf\t", exp(Lik[i][j]-Ltot) );
    }
    fprintf(fe,"\n");
  }
  fclose(fe);
